...
describe('FinancialProfileController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<FinancialProfileController>(
  ...
  describe('create', () => {
    it('should create financial subaccount on Asaas for logged user', async () => {
      ...
      mockFinancialProfileService.createFinancialProfile.mockResolvedValue(
        expected,
      );

      const result = await controller.create(dto as any, req);
      expect(service.createFinancialProfile).toHaveBeenCalledWith(
        dto,
        'user-1',
      );
  ...
  describe('getAllByUserId', () => {
    it('should list financial profiles of the logged user', async () => {
      ...
      mockFinancialProfileService.getAllFinancialProfilesByUserId.mockResolvedValue(
        expected,
      );

      const result = await controller.getAllByUserId(req, undefined);
      expect(service.getAllFinancialProfilesByUserId).toHaveBeenCalledWith(
        'user-1',
        undefined,
      );
      expect(result).toEqual(expected);
  ...
  describe('getAllProfiles', () => {
    it('should list all financial profiles for admin', async () => {
      ...
      mockFinancialProfileService.getAllFinancialProfiles.mockResolvedValue(
        expected,
      );
  ...
  describe('getByUserId', () => {
    it('should return specific financial profile of the logged user', async () => {
      const req = { user: { sub: 'user-1' } } as any;
      const expected = { id: 'fp-1', name: 'Barbearia VIP' };
      mockFinancialProfileService.getFinancialProfileByUserId.mockResolvedValue(
        expected,
      );

      const result = await controller.getByUserId(req, 'fp-1');
      expect(service.getFinancialProfileByUserId).toHaveBeenCalledWith(
        'user-1',
        'fp-1',
      );
      expect(result).toEqual(expected);
  ...
  describe('deactivate', () => {
    it('should deactivate financial profile', async () => {
      ...
      mockFinancialProfileService.deactivateFinancialProfile.mockResolvedValue(
        expected,
      );

      const result = await controller.deactivate('fp-1', req);
      expect(service.deactivateFinancialProfile).toHaveBeenCalledWith(
        'fp-1',
        'user-1',
      );
      expect(result).toEqual(expected);
  ...
  describe('activate', () => {
    it('should activate financial profile', async () => {
      ...
      mockFinancialProfileService.activateFinancialProfile.mockResolvedValue(
        expected,
      );

      const result = await controller.activate('fp-1', req);
      expect(service.activateFinancialProfile).toHaveBeenCalledWith(
        'fp-1',
        'user-1',
      );
      expect(result).toEqual(expected);
  ...
  describe('getFinancialProfileBalance', () => {
    it('should return balance from Asaas subaccount', async () => {
      ...
      mockFinancialProfileService.getFinancialProfileBalance.mockResolvedValue(
        expected,
      );

      const result = await controller.getFinancialProfileBalance('fp-1', req);
      expect(service.getFinancialProfileBalance).toHaveBeenCalledWith(
        'fp-1',
        'user-1',
      );
      expect(result).toEqual(expected);
