...
## 2026-03-09 - [HIGH] Fix Overly Permissive CORS Configuration
**Vulnerability:** The CORS configuration in `src/main.ts` incorrectly allowed any origin using `*` (if provided in env) and any origin matching the regex `/\.vercel\.app$/`, combined with `credentials: true`. This was overly permissive and would allow attackers hosting sites on `.vercel.app` or anywhere else (if `*` was active) to make authenticated cross-origin requests and read sensitive responses.
**Learning:** Hardcoding regex-based origin matching for hosting providers like Vercel is dangerous, as any user can spin up a `.vercel.app` subdomain. This pattern circumvents the primary defense-in-depth benefit of CORS.
**Prevention:** Never use regular expressions for shared domains like `.vercel.app` in CORS origin checks. Ensure only explicit, exact-match trusted domains are included in the allowed origins array.
