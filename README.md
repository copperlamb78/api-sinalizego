...
- **Segurança HTTP, Rate Limiting & Versionamento:** Headers de segurança via `helmet` (CSP, HSTS, X-Frame-Options), controle estrito de CORS por lista de origens (`CORS_ORIGINS`), proteção anti-brute force e anti-flood via `@nestjs/throttler` (limites específicos para login, criação de conta, recuperação de senha e emissão de Pix), prefixo global versionado `/api/v1` (com bypass de webhooks Asaas) e Swagger restrito a ambientes de desenvolvimento/staging (`NODE_ENV !== 'production'`).
